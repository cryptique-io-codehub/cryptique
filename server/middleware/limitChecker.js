...
const checkResourceLimit = (resourceType, increment = 1) => {
  return async (req, res, next) => {
    try {
      // Need a team on the request (should be attached by subscriptionCheck middleware)
      if (!req.team) {
        return res.status(400).json({ error: 'Team information not found. Make sure to use subscriptionCheck middleware first.' });
      }
