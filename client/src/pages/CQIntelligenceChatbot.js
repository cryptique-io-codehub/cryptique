...
const CQIntelligenceChatbot = ({ analytics, websites }) => {
  ...
  const getSelectedSiteAnalytics = () => {
    if (!analytics || !Array.isArray(websites)) return null;
    // If analytics is an array of objects per site
    if (Array.isArray(analytics)) {
      return analytics.find(a => a.siteId === selectedSite) || null;
    }
    // If analytics is an object keyed by siteId
    if (typeof analytics === 'object' && analytics !== null) {
      return analytics[selectedSite] || null;
    }
    return null;
  };
