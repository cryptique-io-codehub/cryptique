...
const Sidebar = ({ isOpen, onClose, hideMarketing }) => (
  <aside
    className={`fixed md:relative bg-white p-2 shadow-lg flex flex-col h-screen border-r transform ${
      isOpen ? "translate-x-0 w-full md:w-48 top-0 left-0 md:relative md:top-auto md:left-auto" : "-translate-x-full md:translate-x-0 w-48"
    } md:translate-x-0 md:w-48 transition-transform duration-300 ease-in-out z-50 md:z-40 md:absolute md:h-full md:z-50`}
  ...
  >
    <div className="flex justify-between items-center mb-2">
      ...
      <button className="md:hidden" onClick={() => { onClose(); hideMarketing(); }}>✖</button>
    </div>
