/**
 * ethereumChain.js - Module for handling Ethereum chain transactions
 * Uses Etherscan API to fetch transactions for a given address
 */
...
const ETHERSCAN_ACCT_TX_ENDPOINT = `${ETHERSCAN_API_URL}?module=account&action=txlist`;
...
export const fetchEthereumTransactions = async (contractAddress, options = {}) => {
  try {
    ...
    console.log(`Fetching up to ${limit} latest transactions from Etherscan for contract: ${contractAddress}, starting from block ${startBlock}`);
      ...
      while (!success && retryCount < maxRetries) {
        try {
          ...
          if (response.data.status === "0") {
            ...
            console.log(`Etherscan API error: ${errorMsg} (attempt ${retryCount + 1}/${maxRetries})`);
